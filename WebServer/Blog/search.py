import time
from .models import News, NewsCategory, NewsWord
...
# Stop words are from https://github.com/goto456/stopwords/blob/master/baidu_stopwords.txt
...
def GenerateIndex(request):
    return HttpResponse("Index generated forbidden")

    # Due to performance issues this cannot be done like this.
    # NewsWord.objects.all().delete()
    # for news in News.objects.all():
    #     print(news.document_id, end=': ')
    #     content = ImagePlaceholderRe.sub('', news.content)
    #     words = set(jieba.cut(content)) - StopWords
    #     for word in words:
    #         NewsWord(word=word, news=news).save()

    #     print(len(words))

    cursor = connection.cursor()
    cursor.execute('DELETE FROM Blog_newsword')
    cursor.close()

    cursor = connection.cursor()
    newsList = cursor.execute('SELECT id, title, content FROM Blog_news').fetchall()
    cursor.close()
    for news in newsList:
        print(news[0], end=': ')
        content: str = news[1] + ImagePlaceholderRe.sub('', news[2]).replace("'", "")
        words = set(jieba.cut_for_search(content.lower())) - StopWords
        query = ','.join([f"('{word}', {news[0]})" for word in words])

        cursor = connection.cursor()
        cursor.execute('INSERT INTO Blog_newsword (word, news_id) VALUES ' + query)
        cursor.close()

        print(len(words))

    return HttpResponse("Index generated successfully")


def InitSearchIndex(request):
    global ReverseDict
    ReverseDict = {}
    cursor = connection.cursor()
    for row in cursor.execute("SELECT word, group_concat(news_id) FROM Blog_newsword GROUP BY word"):
        ReverseDict[row[0]] = list(map(int, row[1].split(',')))

    print('Search index initialized')


def PerformSearch(request) -> (HttpResponseRedirect, dict):
    assert request.method == 'POST'

    mode = request.POST.get('mode')
    mode = 1 if mode == '1' else 0

    categories = request.POST.getlist('type')
    if len(categories) == 0:
    ...
    if mode == 0:
        ...
        # May cause SQL injection? But due to simplicity, I will not fix it
        result = News.objects.raw(
            f"SELECT * FROM Blog_news WHERE category IN ({categories}) AND (title LIKE '%%{keyword}%%' OR content LIKE '%%{keyword}%%') ORDER BY {sort} DESC")

        searchStopTime = time.time()

        result = {
            'time': '%.3f' % ((searchStopTime - searchStartTime) * 1000),
            'data': result
        }

        return (HttpResponseRedirect(reverse('list') + f"?search=1"), result)

    else:
        if ReverseDict is None:
            InitSearchIndex(request)
        ...
        searchStartTime = time.time()
        keywords = set(jieba.cut_for_search(request.POST.get('search').lower())) - StopWords

        if len(keywords) == 0:
            result = News.objects.filter(category__in=categories).order_by(sort)

        else:
            w = keywords.pop()
            if w not in ReverseDict:
                newsSet = set()
            else:
                newsSet = set(ReverseDict.get(w))
                ...
                for word in keywords:
                    ...
                    newsSet &= set(ReverseDict.get(word))

                result = News.objects.filter(id__in=newsSet).filter(category__in=categories).order_by(sort)
