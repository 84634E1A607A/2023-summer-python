...
CREATE TABLE IF NOT EXISTS words (
    ...
    docid TEXT NOT NULL,
    CONSTRAINT fk_words_docid FOREIGN KEY (docid) REFERENCES news (docid)
)
''')

ImagePlaceholderPattern = re.compile(r'<_IMAGE_(.*?)_/>')

def ExtractWords(docid: str, content: str) -> None:
    words = jieba.cut(ImagePlaceholderPattern.sub("", content.lower()))
    ...
    for word in wordsDict:
        dbconn.execute(f'''\
INSERT OR IGNORE INTO words (word, count, docid)
VALUES (
    "{word}",
    "{wordsDict[word]}",
    "{docid}"
...
''')
    ...
    tags = dbconn.execute(f'SELECT tags FROM news WHERE docid = "{docid}"').fetchone()[0]
    ...
    keywords = set(tags)
    for word in wordsDict:
        if word[0] not in tags and len(word[0]) > 1:
            keywords.add(word[0])
        if len(keywords) >= 8:
            break
    ...
    dbconn.execute(f'UPDATE news SET keywords = "{keywords}", words_extracted = 1 WHERE docid = "{docid}"')

    dbconn.commit()
    print(f'Extracted {len(wordsDict)} words from {docid}.')

def main():
    cursor = dbconn.execute('SELECT news.docid, contents.content FROM news INNER JOIN contents ON news.docid = contents.docid WHERE news.words_extracted = 0')
    for index, row in enumerate(cursor):
        print(f'[{index}]')
        ExtractWords(row[0], row[1])
